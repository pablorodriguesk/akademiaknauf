...
class Login extends CI_Controller
{
    ...
    public function validate_login($from = "")
    {
        ...
        //redirect here
        $pass = $this->db-> select('password, email')-> where('email', $email)-> get('users')-> row();


        if(empty($pass->password)  && (!empty($pass->email)) ){
            redirect(site_url('login/first_access/?email='.base64_encode('xptop3520|'.$email)), 'refresh');
        }
    ...
    function first_access(){
        $email = str_replace("xptop3520|","",base64_decode($this->input->get('email'))); 
        $page_data['page_name'] = 'first_access';
        $page_data['page_title'] = site_phrase('first_access');
        $page_data['page_email'] = $email;
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    ...
    public function register()
    {
        ...
        $data['idade']  = html_escape($this->input->post('idade'));
        $data['estado']  = html_escape($this->input->post('UF'));
        $data['cidade']  = html_escape($this->input->post('cidade'));
        $data['origem']  = html_escape($this->input->post('origem'));
        $data['empresa']  = html_escape($this->input->post('empresa'));
        $data['profissao']  = html_escape($this->input->post('profissao'));
