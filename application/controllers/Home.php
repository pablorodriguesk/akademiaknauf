...
class Home extends CI_Controller
{
    ...
    public function course($slug = "", $course_id = "")
    {
       ...
       // $tipo = $this->akademia_model->tipo($course_id);
    ...
    public function inscricao_history()
    {
        if ($this->session->userdata('user_login') != true) {
            redirect(site_url('home'), 'refresh');
        }

        /*$total_rows = $this->akademia_model->inscricoes($this->session->userdata('user_id'))->num_rows();
        $config = array();
        $config = pagintaion($total_rows, 10);
        $config['base_url']  = site_url('home/inscricao_history');
        $this->pagination->initialize($config);
        $page_data['per_page']   = $config['per_page'];*/

        $page_data['page_title'] = site_phrase('Inscrições');
        $page_data['page_name']  = "inscricao_history";
        $this->load->view('frontend/' . get_frontend_settings('theme') . '/index', $page_data);
    }
    ...
    public function get_enrolled_to_free_coursep($course_id,$idturma)
    {
        $course_details = $this->crud_model->get_course_by_id($course_id)->row_array();
    ...
    public function iturma($idturma)
    {
        ...
        if ($this->session->userdata('user_login') != true) {
            ...
            $inscricao["id_turma"] = (int)$turma->row('id');
